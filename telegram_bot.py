...
async def _handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    progress_enabled = bool(tg.get("progress_updates", False))
    progress_events: list[str] = []

    def _on_progress(event: str) -> None:
        if progress_enabled and len(progress_events) < 8:
    ...
    try:
        response = await agent.run(user_text, on_event=_on_progress)
    except Exception as exc:
        logger.exception("Agent error in Telegram handler")
        response = f"Sorry, I encountered an error: {exc}"

    if progress_enabled and progress_events:
